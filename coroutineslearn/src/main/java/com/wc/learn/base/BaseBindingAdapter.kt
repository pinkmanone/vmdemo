...
abstract class BaseBindingAdapter<T, VDB : ViewDataBinding>(@LayoutRes val id: Int) :
    RecyclerView.Adapter<BaseBindingAdapter<T, VDB>.BaseBindingViewHolder>() {
...
//        notifyDataSetChanged()
    ...
    abstract fun convert(binding: VDB?, t: T?, position: Int, listener: OnItemClickListener<T>?)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseBindingViewHolder {
        if (headers.get(viewType) != null) {
            return BaseBindingViewHolder(headers.get(viewType))
        } else if (footers.get(viewType) != null) {
            return BaseBindingViewHolder(footers.get(viewType))
        }

        if (isEmpty() && viewType == ITEM_TYPE_EMPTY) {
            return if (mEmptyView != null) {
                BaseBindingViewHolder(mEmptyView)
            } else {
                BaseBindingViewHolder(parent, mEmptyLayoutId)
            }
        }

        return BaseBindingViewHolder(parent, id)
    }
    ...
    override fun onViewAttachedToWindow(holder: BaseBindingViewHolder) {
    ...
    inner class BaseBindingViewHolder
        ...
        val binding: VDB? = DataBindingUtil.bind<VDB>(itemView)

        fun bindData(t: T?, position: Int, listener: OnItemClickListener<T>?) {
            convert(binding, t, position, listener)
        }
